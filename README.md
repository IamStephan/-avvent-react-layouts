...
| display     |        `'flex'`         |                      `inline-flex | flex`                       |     -      | should the component be inline or not                                                                                                   |
| flow        |         `'row'`         |          `row | row-reverse | column | column-reverse`          |     -      | Direction the parent should lay its children                                                                                            |
| wrap        |       `'nowrap'`        |                 `wrap | nowrap | wrap-reverse`                  |     -      | should the children wrap if they overlap                                                                                                |
| justify     |     `'flex-start'`      | `flex-start | flex-end | center | space-around | space-between` |     -      | How the children should align on its main axis                                                                                          |
| align       |     `'flex-start'`      |      `flex-start | flex-end | center | stretch | baseline`      |     -      | How the children should align on its cross axis                                                                                         |
| content     |     `'flex-start'`      |      `flex-start | flex-end | center | stretch | baseline`      |     -      | When there is space how should all the content be aligned on its cross-axis                                                             |
...
| debug        |         `false`         |                  `bool`                   |     -      | When set to `true` the parent component will have a grey background color with an opacity of 0.25 and the box component a dashed border |
| breakPoints  | `{default breakpoints}` |                 `object`                  |     -      | Breakpoints for responsive design                                                                                                       |
| display      |        `'grid'`         |           `grid | inline-grid`            |     -      | Whether the container should be an inline grid or just grid                                                                             |
| columns      |         `null`          |             `string | array`              | columns-*  | The grid template columns (How many columns should be in the matrix)                                                                    |
| auto-columns |         `null`          |                 `string`                  |     -      | Auto column width if the grid flow is in row mode, (Just a convenience prop)                                                            |
| rows         |         `null`          |             `string | array`              |   rows-*   | The grid template rows (How many rows should be in the matrix)                                                                          |
...
| justify      |       `'stretch'`       |     `start | end | center | stretch`      |     -      | How the content inside a grid cell should be aligned on its main axis                                                                   |
| align        |       `'stretch'`       |     `start | end | center | stretch`      |     -      | How the content inside a grid cell should be aligned on its cross axis                                                                  |
| flow         |         `'row'`         | `row | column | row dense | column dense` |            | The direction the grid flows in                                                                                                         |
| children     |                         |                   `Box`                   |            | All the children of this component has to be a box component                                                                            |
...
| flexAlign |   `'auto'`    | `auto | flex-start | flex-end | center | baseline | stretch` |            | How the box should be aligned if there is space available                                                            |
...
| Prop   | Default value |     Value Type      | Smart prop | Description                                                                                                          |
| :----- | :-----------: | :-----------------: | :--------: | :------------------------------------------------------------------------------------------------------------------- |
| column |    `null`     | `[string] | string` |  column-*  | How many columns the component should occupy(syntax: ['', '']). If only one value is used them it should be a string |
| row    |    `null`     | `[string] | string` |   row-*    | How many rows the component should occupy(syntax: ['', '']). If only one value is used them it should be a string    |
